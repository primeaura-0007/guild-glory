...
# Database: use Vercel Postgres if DATABASE_URL is set, otherwise fallback to SQLite (local dev)
...
def admin_update_group_auto_glory(admin):
    ...
    if enabled:
        group.auto_glory_last_run = datetime.now(timezone.utc)
...
def api_glory_progression(user):
    ...
    for i in range(5):
        snapshots.append({
            'captured_at': (datetime.now(timezone.utc) - timedelta(minutes=i*15)).isoformat(),
...
# ---------- Auto Glory Cron Endpoint (replaces background thread) ----------
@app.route('/api/cron/auto-glory', methods=['POST'])
def auto_glory_cron():
    """
    Call this endpoint every few minutes using a free cron service (e.g., cron-job.org).
    It processes all groups that have auto_glory_enabled=True and are running.
    """
    # Optional security: set CRON_SECRET in Vercel environment and verify header
    ...
    db.session.commit()
    return jsonify({'success': True, 'updated_groups': updated_count}), 200
